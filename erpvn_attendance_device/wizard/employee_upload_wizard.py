...
class EmployeeUploadWizard(models.TransientModel):
    ...
    device_ids = fields.Many2many('attendance.device', 'employee_upload_wizard_attendance_device_rel', 'wizard_id', 'device_id',
        string='Devices', required=True, compute='_compute_devices', store=True, readonly=False)
    employee_ids = fields.Many2many('hr.employee', 'employee_upload_wizard_hr_employee_rel', 'wizard_id', 'employee_id',
        string='Employees to upload', required=True)
    ...
    def prepare_lines(self):
        ...
        for employee in self.employee_ids:
            ...
            employee_id = isinstance(employee.id, models.NewId) and employee.id.origin or employee.id
            for device in self.device_ids:
                device_id = isinstance(device.id, models.NewId) and device.id.origin or device.id
                new_line = (0, 0, {
    ...
    def _compute_line_ids(self):
        for record in self:
            record.line_ids = [(5,)] + record.prepare_lines()
...
class EmployeeUploadLine(models.TransientModel):
    ...
    def upload_employees(self):
        for record in self:
            record.employee_id.upload_user_attendance_device(record.device_id)