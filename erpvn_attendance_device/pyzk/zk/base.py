...
class ZK(object):
    ...
    def get_oem_vendor(self):
        ...
        cmd_response = self.__send_command(command, command_string, response_size)
        if cmd_response.get('status'):
            oem_vendor = self.__data.split(b'=', 1)[-1].split(b'\x00')[0]
            oem_vendor = oem_vendor.replace(b'=', b'')
            return oem_vendor.decode()  # string?
        else:
            raise ZKErrorResponse("Can't read the OEM Vendor of the device")
    ...
    def get_workcode(self):
        ...
        cmd_response = self.__send_command(command, command_string, response_size)
        if cmd_response.get('status'):
            workcode = self.__data.split(b'=', 1)[-1].split(b'\x00')[0]
            return workcode.decode()
        else:
            return ""
