...
class FingerTemplate(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', compute='_compute_employee_id', inverse='_set_employee_id', store=True,
        help='The employee who is owner of this finger template', ondelete='cascade')
    uid = fields.Integer(string='UId', related='device_user_id.uid', store=True,
        help='The ID (technical field) of the user/employee in the device storage')
    device_id = fields.Many2one('attendance.device', string='Attendance Device', related='device_user_id.device_id', store=True)
    ...
    def _compute_employee_id(self):
        for r in self:
            if r.device_user_id and r.device_user_id.employee_id:
                ...
                r.employee_id = res[0] if res else False
