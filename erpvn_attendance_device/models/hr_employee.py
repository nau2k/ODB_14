...
class HrEmployee(models.Model):
    ...
    finger_template_count = fields.Integer(
        string='Finger Templates', compute='_compute_finger_template_count')
    ...
    def _compute_finger_template_count(self):
        for r in self:
            r.finger_template_count = len(r.finger_template_ids)

    @api.model_create_multi
    def create(self, vals_list):
        employees = super(HrEmployee, self).create(vals_list)
        attendance_device_ids = self.env['attendance.device'].sudo(
        ).with_context(active_test=False).search([])
        if attendance_device_ids:
            employees.write({'unamapped_attendance_device_ids': [
                            (6, 0, attendance_device_ids.ids)]})
        return employees
    ...
    def _get_unaccent_name(self):
        return self.env['erpvn.base'].strip_accents(self.name)
    ...
    def upload_user_attendance_device(self, device):
        self.ensure_one()
        if not self.barcode:
            raise ValidationError(
                _("Employee '%s' has no Badge ID specified!"))
        device_user = self.create_device_user_if_not_exist(device)
        if device_user:
            device_user.set_device_user()
