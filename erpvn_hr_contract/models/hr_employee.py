...
class HrEmployee(models.Model):
    ...
    social_insurance_no = fields.Char('Social Insurance')
    health_insurance_no = fields.Char('Health Insurance Number')
    tax_code = fields.Char('Tax Code')

    @api.depends('contract_ids.date_start', 'contract_ids.structure_type_id', 'contract_ids.employee_id')
    def _compute_hire_date(self):
        ...
        for employee in self:
            contracts = contract_obj.search([('employee_id', '=', employee.id),
                ('structure_type_id.count_working_seniority', '=', True)], order='date_start DESC')
            if not contracts:
                continue
            ...
            while len(contracts) > 1:
                if not contracts[1].date_end:
                    ...
                    raise Warning(_("The previous contract (ID: %s, %s) should be ended before create a new contract" % (contracts[1].id, contracts[1].name)))
    ...
    hire_date = fields.Date(compute=_compute_hire_date, store=True, string='Hired Date', groups="hr.group_hr_user",
